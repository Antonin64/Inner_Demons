...
{
    if (tmp->col->on_collide(game, tmp, NULL, COL_PPROJ)) {
...
{
    if (tmp->type == ANIM_OBJ && tmp->col->player_proj == sfTrue
    && are_hitbox_colliding(anim->hitbox, tmp->hitbox)) {
...
int check_anim_col(game_t *game, anim_t *anim)
{
    if (anim && anim->col->map) {
        if (is_point_from_rect_col(game, anim->hitbox->rect, 5))
