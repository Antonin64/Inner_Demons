    ...
    game_t *game)
{
    ...
    if (!strcmp(name, "wp1") || !strcmp(name, "wp2")) {
        if (!strcmp(name, "wp1") && game->menu->f_flag == 2)
            return spawn_weapon(game, pos, create_ak47(game->res), 0);
        if (!strcmp(name, "wp2") && game->menu->f_flag == 2)
            return spawn_weapon(game, pos, create_shotgun(game->res), 0);
